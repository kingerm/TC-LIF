...
class BaseNode(base.MemoryModule):
    ...
    def neuronal_fire(self):
        return self.surrogate_function(self.v - self.v_threshold)  # 判断是否需要输出1
...
class Node1(BaseNode):  # 关键在看懂这个node。其他的都是很简单的神经网络训练过程
    def __init__(self,
                 v_threshold=1.,
                 v_reset=0.,
                 surrogate_function: Callable = None,
                 ...
                 gamma: float = 0.5):
        super(Node1, self).__init__(v_threshold, v_reset, surrogate_function, detach_reset, step_mode)
        self.k = k
        ...
        self.decay_factor = torch.nn.Parameter(decay_factor)  # decay_factor是可以训练的参数
    ...
    def neuronal_charge(self, x: torch.Tensor):
        ...
        self.names['v1'] = torch.sigmoid(self.decay_factor[0][0]) * self.names['v1'] + x  # self.decay_factor[0][0]是beta1
        self.names['v2'] = torch.sigmoid(self.decay_factor[0][1]) * self.names['v2'] + self.names['v1']  # self.decay_factor[0][1]是beta2
        self.v = self.names['v2']  # x是已经经过nn.Linear层的输出，即I(t)
        # self.v记录的是需要和阈值比较的电压
