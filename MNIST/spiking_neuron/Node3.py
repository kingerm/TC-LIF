...
class Node3(BaseNode):  # 关键在看懂这个node。其他的都是很简单的神经网络训练过程
    def __init__(self,
                 v_threshold=1.,
                 v_reset=0.,
                 surrogate_function: Callable = None,
                 ...
                 gamma: torch.Tensor = torch.full([1, 2], 0, dtype=torch.float)):
        super(Node3, self).__init__(v_threshold, v_reset, surrogate_function, detach_reset, step_mode)
        self.k = k
    ...
    def neuronal_charge(self, x: torch.Tensor):
        ...
        self.names['v2'] = self.names['v2'] + torch.sigmoid(self.decay_factor[0][1]) * self.names['v1'] - torch.sigmoid(self.decay_factor[0][2]) * self.names['v3']  # self.decay_factor[0][1]是beta2
        self.names['v3'] = self.names['v3'] + torch.sigmoid(self.decay_factor[0][3]) * self.names['v2']
        self.v = self.names['v3']  # x是已经经过nn.Linear层的输出，即I(t)
    ...
    def neuronal_reset(self, spike):
        ...
        if not self.hard_reset:  # 需要修改这里以及上面的neuronal_charge
            # soft reset
            self.names['v1'] = self.jit_soft_reset(self.names['v1'], spike_d, self.gamma[0][0])  # 这是放电过程。gamma小于1使得树突室为部分放电
            self.names['v2'] = self.jit_soft_reset(self.names['v2'], spike_d, self.gamma[0][1])
