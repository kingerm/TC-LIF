...
class Node4(BaseNode):  # 关键在看懂这个node。其他的都是很简单的神经网络训练过程
    def __init__(self,
                 ...
                 step_mode='s',
                 k=3,
                 decay_factor: torch.Tensor = torch.full([1, 8], 0, dtype=torch.float),  # 这里确实可以全是0
                 gamma: float = 0.5):
        super(Node4, self).__init__(v_threshold, v_reset, surrogate_function, detach_reset, step_mode)
        self.k = k
    ...
    def neuronal_charge(self, x: torch.Tensor):
        ...
        s1 = self.names['v2']
        s2 = self.names['v3']  # 这里的self.names['v1']前面确实不用加可学习的decay项
        self.names['v1'] = self.names['v1'] - torch.sigmoid(self.decay_factor[0][0]) * self.names['v2'] - torch.sigmoid(self.decay_factor[0][1]) * self.names['v3'] + x
        self.names['v2'] = torch.sigmoid(self.decay_factor[0][2]) * self.names['v1'] + torch.sigmoid(self.decay_factor[0][3]) * s1 + torch.sigmoid(self.decay_factor[0][4]) * s2
        self.names['v3'] = torch.sigmoid(self.decay_factor[0][5]) * self.names['v1'] + torch.sigmoid(self.decay_factor[0][6]) * s2 + torch.sigmoid(self.decay_factor[0][7]) * s1
        self.v = self.names['v2'] + self.names['v3']  # x是已经经过nn.Linear层的输出，即I(t)
    ...
    def neuronal_reset(self, spike):
        ...
        if not self.hard_reset:  # 需要修改这里以及上面的neuronal_charge
            # soft reset
             # 这是放电过程。gamma小于1使得树突室为部分放电
            self.names['v1'] = self.jit_soft_reset(self.names['v1'], spike_d, self.gamma)
            self.names['v2'] = self.jit_soft_reset(self.names['v2'], spike_d, self.v_threshold[0][0])
            self.names['v3'] = self.jit_soft_reset(self.names['v3'], spike_d, self.v_threshold[0][1])
